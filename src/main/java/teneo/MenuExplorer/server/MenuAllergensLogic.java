...
/**
 * Service class responsible for loading, parsing, and querying allergen and ingredient information
 * from a structured JSON file. Supports smart search capabilities via MenuSmartSearch.
 */
@Service
public class MenuAllergensLogic implements IAllergen {
    ...
    /**
     * Loads allergen data from a JSON file and populates smart search index if enabled.
     *
    ...
    public String searchAllergens(String query) {
        if (MenuSmartSearch.searchEnabled()) {
        	String name = MenuSmartSearch.match(query, allergenMenu).name;
            return name + ": " + getAllergens(name).toString();
        }
        return null;
    }
    ...
    public String searchIngredients(String query) {
        if (MenuSmartSearch.searchEnabled()) {
            return getIngredients(MenuSmartSearch.match(query, allergenMenu).name).toString();
        }
        return null;
    }
